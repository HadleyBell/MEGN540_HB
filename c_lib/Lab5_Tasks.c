...
void Send_Encoders_velocity( float __unused)
{
    ...
    if( Battery_Recent() > BATTERY_MOTOR_MIN_VOLTAGE ) {
        ...
        float left_vel = Controller_Update(&controller_left, left_meas, left_controller.update_period);
        ...
        float right_vel = Controller_Update(&controller_right, right_meas, right_controller.update_period);
